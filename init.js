  ...
  const upcomingWorkshops = WORKSHOPS.filter(
    (w) =>
      ...
      (w.month > currentMonth ||
        (w.month == currentMonth && w.day >= currentDate)),
  );
...
const loadPastEvents = () => {
  ...
  setInterval(() => {
    ...
    const scrollIdx =
      Math.round(pastWorkshopContainer.scrollLeft / slideWidth) + 1;
    pastWorkshopContainer.scrollTo({
      left: (scrollIdx % slideIds.length) * slideWidth,
    });
  }, 5000);
};
